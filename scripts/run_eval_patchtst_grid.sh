...
# ==============================================================================
# Train Prompt-Z and evaluate frozen-vs-PZ for PatchTST on all datasets with
# H in {1,12,24,48}.
#
...
#
# Outputs:
#   logs/prompt_z_grid/patchtst_h1_12_24_48/summary.tsv
#
# Usage:
#   bash scripts/run_eval_patchtst_grid.sh
#
# Useful overrides:
#   RUN_TAG=strict_label_v1 FORCE_TRAIN=1 bash scripts/run_eval_patchtst_grid.sh
#   PZ_MODE=mode1 bash scripts/run_eval_patchtst_grid.sh
#   DATASETS="ECL.csv Traffic.csv" HORIZONS="1 12" bash scripts/run_eval_patchtst_grid.sh
# ==============================================================================
...
LOGDIR=${LOGDIR:-logs/prompt_z_grid/patchtst_h1_12_24_48}
...
find_backbone_weights() {
    ...
    local ga="$3"
    local pats=(
        "weights/patchtst_pretrained_${ds}_H${h}_GA${ga}.pth"
        "weights/patchtst_pretrained_${ds}_H${h}_GA1.pth"
        "weights/patchtst_pretrained_${ds}_H${h}.pth"
        "weights/backbone/patchtst_pretrained_${ds}_H${h}_GA${ga}.pth"
        "weights/backbone/patchtst_pretrained_${ds}_H${h}_GA1.pth"
        "weights/backbone/patchtst_pretrained_${ds}_H${h}.pth"
        "weights/pretrained/patchtst_pretrained_${ds}_H${h}_GA${ga}.pth"
        "weights/pretrained/patchtst_pretrained_${ds}_H${h}_GA1.pth"
        "weights/pretrained/patchtst_pretrained_${ds}_H${h}.pth"
    )
...
train_prompt_z_case() {
    ...
    PYTHONPATH=. "${PYTHON_BIN}" train_prompt_z.py \
        --root_path "${ROOT_PATH}" \
        --data_path "${data}" \
        --features M \
        --seq_len "${SEQ_LEN}" \
...
run_eval_case() {
    ...
    echo -n "[EVAL] ${tag} ..." >&2
    PYTHONPATH=. "${PYTHON_BIN}" main_prompt_z.py "${args[@]}" > "${log}" 2>&1
...
echo "================================================================="
echo " Prompt-Z Train + Eval: PatchTST"
echo "================================================================="
