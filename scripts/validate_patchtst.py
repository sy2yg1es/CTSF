"""
validate_patchtst.py — PatchTST: Train Prompt-Z → Frozen Eval → PZ Eval
=========================================================================

对所有数据集 × H ∈ {1, 12, 24, 48}:
  1. 训练 Prompt-Z modulator（backbone 冻结）
  2. 跑 frozen baseline streaming eval（测试集）
  3. 跑 Prompt-Z mode0 streaming eval（测试集）
  4. 汇总对比 frozen vs PZ 的 MSE 提升

训练 step 打印格式:
  [Epoch E Step S] loss=... forecast=... frozen=... gamma=... mask_mean=...
                   mask_ratio=... reg_scale=... noop=... noop_scale=...
                   gamma_floor=... residual_cache_len=... residual_tracker_warmed=...
                   raw_d/h=... eff_d/h=...

用法:
    python scripts/validate_patchtst.py
    python scripts/validate_patchtst.py --horizons 1 12 --datasets ECL.csv
    FORCE_TRAIN=1 python scripts/validate_patchtst.py
"""
...
TRAIN_LOG_INTERVAL   = 200   # step 打印间隔（训练）
EVAL_LOG_INTERVAL    = 500   # step 打印间隔（评估）
...
# 模型构建
# ══════════════════════════════════════════════════════════════════════════
def build_backbone(enc_in: int, forecast_H: int, device: torch.device) -> PatchTSTAdapter:
...
def load_backbone_weights(adapter: PatchTSTAdapter, weight_path: str, device: torch.device) -> None:
...
# 数据加载
# ══════════════════════════════════════════════════════════════════════════
def _dp_args(root_path, data_path, forecast_H):
    class A:
        pass
    a = A()
...
def get_train_dataloader(root_path, data_path, forecast_H):
    dataset, _ = data_provider(_dp_args(root_path, data_path, forecast_H))
    train_size  = dataset.train_size
    subset = Subset(dataset, range(train_size))
    print(f"  [*] Train subset: {train_size}/{len(dataset)} windows "
          f"(raw_train_end={dataset.raw_train_end})")
    loader = DataLoader(
...
def get_streaming_loader(root_path, data_path, forecast_H):
    dataset, base = data_provider(_dp_args(root_path, data_path, forecast_H))
    streaming = StreamingEnvironment(base, forecast_H=forecast_H)
...
def train_prompt_z(
    ...
    """训练 Prompt-Z，返回最终权重路径。若已存在且 force=False 则跳过。"""
...
def run_streaming_eval(
    ...
    返回 {MSE, MAE, RMSE, n_aligned}
    """
    dataset, streaming_loader = get_streaming_loader(root_path, data_path, forecast_H)
    test_start = dataset.test_start
    N_total    = len(dataset)

    print(f"  [*] [{label}] mode={mode} | total={N_total} | test_start={test_start} | "
    ...
    for t, (X_t, Y_arrived) in enumerate(streaming_loader):
        ...
        # mode1 calibration
        if mode == "mode1" and t >= test_start and calib_optimizer is not None:
        ...
        if mode == "mode1" and t >= test_start and calib_optimizer is not None:
            ...
            hc = hc.detach(); mc = mc.detach(); sc = sc.detach()
        ...
        if t % EVAL_LOG_INTERVAL == 0 and n_aligned > 0:
            cur_mse = (mse_sum / n_aligned).item()
            print(f"    [Step {t}] MSE={cur_mse:.6f} n={n_aligned}")

    elapsed = time.time() - t0
    if n_aligned == 0:
        print(f"  [!] [{label}] No aligned test steps!")
        return {"MSE": float("inf"), "MAE": float("inf"), "RMSE": float("inf"), "n_aligned": 0}

    final_mse  = (mse_sum / n_aligned).item()
    final_mae  = (mae_sum / n_aligned).item()
    final_rmse = final_mse ** 0.5
    print(f"  [*] [{label}] n={n_aligned} | MSE={final_mse:.6f} | "
          f"MAE={final_mae:.6f} | RMSE={final_rmse:.6f} | {elapsed:.1f}s")
...
def run_streaming_eval_splits(
    ...
    """
    Run a causal streaming pass and report metrics by prediction-window index.
    The delayed label at stream time t belongs to prediction index t - H.
    """
    dataset, streaming_loader = get_streaming_loader(root_path, data_path, forecast_H)
    n_total = len(dataset)
    if splits is None:
...
# ══════════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="PatchTST: Train Prompt-Z + Full Eval")
    parser.add_argument("--root_path",   type=str,   default=None)
    parser.add_argument("--datasets",    nargs="+",  default=DEFAULT_DATASETS, metavar="DS")
    parser.add_argument("--horizons",    nargs="+",  type=int, default=DEFAULT_HORIZONS, metavar="H")
    parser.add_argument("--weight_dir",  type=str,   default="weights")
    parser.add_argument("--pz_dir",      type=str,   default="weights/prompt_z",
                        help="Prompt-Z 权重保存目录")
    parser.add_argument("--logdir",      type=str,   default="logs/validate/patchtst_h1_12_24_48")
    ...
    print("=" * 70)
    print(f"  PatchTST  Train Prompt-Z → Frozen Eval → {args.pz_mode.upper()} Eval")
    print("=" * 70)
    print(f"  root_path  : {args.root_path}")
    print(f"  datasets   : {args.datasets}")
    print(f"  horizons   : {args.horizons}")
    print(f"  pz_mode    : {args.pz_mode}")
    print(f"  weight_dir : {args.weight_dir}  pz_dir: {args.pz_dir}")
    ...
    for data_path in args.datasets:
        ...
        for h in args.horizons:
            case = f"{ds}_H{h}"
            print(f"\n{'='*70}")
            print(f"[CASE] {case}  (enc_in={enc_in})")
            print(f"{'='*70}")

            # ── 找 backbone 权重 ──────────────────────────────────────────
            wpath = find_backbone_weights(ds, h, args.weight_dir)
            if wpath is None:
                print(f"  [MISSING] No backbone weights for {case}")
            ...
            load_backbone_weights(adapter, wpath, device)
            ...
            # 重建模型（重置 residual tracker + 重新加载 PZ 权重）
            adapter2 = build_backbone(enc_in, h, device)
            load_backbone_weights(adapter2, wpath, device)
            ...
            frozen_mse = frozen_test["MSE"]
            pz_mse = pz_test["MSE"]
            delta_pct = test_improve
            ...
            print(f"\n  [TEST] {case} frozen_MSE={frozen_mse:.6f} "
                  f"pz_MSE={pz_mse:.6f} improve={delta_pct:+.2f}%")
